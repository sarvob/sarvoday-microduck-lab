#!/usr/bin/env python3
"""Attempt 6: locally refine the reproducible joint-walk controller.

Only candidates that retain all three harbor gates can outrank the starting
controller. Within that feasible set, search maximizes worst-seed chop survival.
The official walking policy remains frozen and surge remains held out.
"""
...
START_PATH = ROOT / "artifacts" / "012-variable-speed-boat-balance" / "joint-walk-continuation-result.json"
OUT_PATH = ROOT / "artifacts" / "012-variable-speed-boat-balance" / "joint-walk-refinement-result.json"
...
def evaluate(sim: Microduck, profiles: list[dict], seeds: list[int],
    ...
    feasibility = 1000.0 if harbor_passes == 3 else 200.0 * harbor_passes
    score = (
        feasibility + 200.0 * chop_passes
        + 40.0 * min(chop_survival) + 6.0 * float(np.mean(chop_survival))
        - 0.01 * float(np.sum(parameters ** 2))
    )
    return score, rows
...
def main() -> int:
    ...
    mean = start.copy()
    sigma = np.r_[np.full(8, 0.16), [0.12, 0.05, 0.12, 0.05]]
    ...
    for generation in range(12):
        ...
        sigma = np.maximum(elite.std(axis=0), 0.012)
    ...
    result = {
        ...
        "objective": "hard-preserve all harbor gates, then maximize worst-seed chop survival",
