...
for f in "$here"/narration-parts/{01,02,03,04,05,06,07}.wav \
  ...
  "$story"/{baseline-hard-goal,baseline-center-goal,baseline-wide-goal,predictor-hard-save,predictor-center-save,predictor-wide-save}.mp4; do
...
encode_overlay "$story/baseline-hard-goal.mp4" "$here/cold-title.png" 12 01-the-bet
...
encode_video "$story/baseline-center-goal.mp4" 13 06-almost-right 0.5
encode_overlay "$story/baseline-center-goal.mp4" "$here/baseline-title.png" 15 07-baseline-good 1.1
encode_video "$story/baseline-wide-goal.mp4" 15 08-baseline-more 0
encode_video "$story/baseline-hard-goal.mp4" 14 09-baseline-fails 0
...
encode_video "$story/baseline-center-goal.mp4" 12 14-training-reactive 0
...
encode_still "$here/result.png" 18 24-result
encode_video "$story/predictor-hard-save.mp4" 14 25-victory 0.5
encode_portrait_broll "$here/stock-broll/pexels-6084018.mp4" 7 26-keeper-button 1.0
...
ffmpeg -y -hide_banner -loglevel error -f concat -safe 0 -i "$list" \
  -i "$here/narration-v3.wav" -i "$here/soundtrack.wav" \
  -filter_complex "[2:a]apad=pad_dur=330,atrim=duration=330,volume=0.20[m];[1:a]asplit=2[n1][n2];[m][n1]sidechaincompress=threshold=0.025:ratio=7:attack=18:release=420[ducked];[ducked][n2]amix=inputs=2:duration=first:normalize=0,loudnorm=I=-15.5:TP=-1.2:LRA=10[a]" \
  -map 0:v:0 -map "[a]" -t 330 -c:v copy -c:a aac -b:a 256k -ar 48000 \
  -movflags +faststart "$here/episode-006-goalkeeper-story-v5.mp4"

printf '%s\n' "$here/episode-006-goalkeeper-story-v5.mp4"
