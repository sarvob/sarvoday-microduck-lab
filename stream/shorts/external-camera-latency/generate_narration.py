#!/usr/bin/env python3
"""Generate timed Heart narration one beat at a time."""
...
def main() -> None:
    ...
    sample_rate = 24000
    total = np.zeros(round(52.0 * sample_rate), dtype=np.float32)
    manifest = []
    for index, beat in enumerate(beats, 1):
        ...
        if end > len(total):
            raise RuntimeError(f"beat {index} overruns the 52-second master")
